...
If a V2 version of the software was made, the following changes would be useful:
- Improved efficiency through the use of interrupt-based scheduling and fast-to-sleep.
- Switching to a communication protocol that allows for serial data transmission such as UART or SPI
- Understanding the MPLAB/PIC18F compiler better to allow for common optimiziations that were ommited due to compiler trouble, such as heavier use of case statements instead of if statements to compile to more efficient jump tables, local variable declarations for reduced memory consumption and using two-bit datatypes for flags instead of the 8-bits currently used to further reduce memory consumption.
- Heartbeat signals from the sensor boards which can be interpretted and used for both debugging and more complicated UI/control schemes, allowing the controller to identify which board is in which header slot arbitrarily.
